...
Once you have your app ID and Secret, use them in `youtube_oauth()` like the following:
```r
youtube_oauth(appId = YOURAPPID, appSecret = YOURAPPSECRET)
```
 ...
 * Get Top Countries
   ...
   * `playlist_top_countries()`
   * `video_top_countries()`
   
 * Get Top Demographics (Age & Gender)
   * `channel_top_demographics()`
   * `playlist_top_demographics()`
   * `video_top_demographics()`
   
 * Get Top Devices
   * `channel_top_devices()`
   * `playlist_top_devices()`
   * `video_top_devices()`
   
 * Get Top Traffic Sources
   * `channel_top_traffic_sources()`
   * `playlist_top_traffic_sources()`
   * `video_top_traffic_sources()`
   
 * Get Top External Traffic Sources
   * `channel_top_external_sources()`
   * `playlist_top_external_sources()`
   * `video_top_external_sources()`
